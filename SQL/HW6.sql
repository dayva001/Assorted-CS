...
#Q1
SELECT 'A' as Source, CONCAT(au_fname,' ', au_lname) as Name, city, state
FROM Authors
UNION
...
FROM Titles T
UNION
SELECT CONCAT(Type,' ', 'Total'), COUNT(type)
FROM Titles CT
GROUP BY Type
ORDER BY Type;
...
SELECT DISTINCT 
  (Select pub_name 
   ...
   where exists (
     Select pub_id 
       from Titles T 
      where T.pub_ID = P.Pub_ID and T.title_id = TA.title_id))as Publisher
   , A.au_fname AS 'First Name', A.au_lname AS 'Last Name'
  FROM  Authors A
  Inner join Title_Authors TA on TA.Au_ID = A.Au_Id;
  
  #Q4
  #For each book type, report the title name, the number of pages of the title,
  #the average number of pages for all books for this book type,
  #and the difference between the two. Use correlated sub-queries in the SELECT clause
  SELECT type AS 'Book Type', title_name AS 'Title', pages, FORMAT((SELECT AVG(pages) FROM Titles AV WHERE T.type = AV.type),0) AS 'Average pages', FORMAT(pages-(SELECT AVG(pages) FROM Titles AV WHERE T.type = AV.type),0) AS 'Difference'
  FROM Titles T;
  
  #Q5
  SELECT T.type AS 'Book Type', title_name AS 'Title', pages, FORMAT(Average,0), FORMAT(pages-Average,0) AS 'Difference'
  FROM Titles T INNER JOIN (SELECT AVG(Pages) AS Average, type FROM Titles GROUP BY type) ST
  ON T.type = ST.type;