...
#HW9
#Q1
CREATE OR REPLACE VIEW ArtistsNoTitles AS
SELECT CONCAT_WS(au_fname, au_lname) AS 'Name'
FROM Authors
...
SELECT DISTINCT CONCAT(au_fname, ' ', au_lname) AS 'FUll Name', title_name, royalty_rate
FROM Authors A INNER JOIN Title_Authors TA
ON A.au_id = TA.au_id
INNER JOIN Royalties R ON TA.title_id = R.title_id
