...
SELECT title_name
FROM Titles
WHERE datediff(Now(), pubdate)/365 <= 10 AND Month(pubdate)=05;

#Q2
SELECT concat_ws('. ', LEFT(au_fname,1) , au_lname)
FROM Authors
WHERE Length(au_fname) >0;

#Q3
SELECT Left(pub_name, Locate('Press', pub_name)-2) AS 'Publisher Name'
FROM Publishers
WHERE Locate('Press', pub_name) >0;
...
#Q5
SELECT Au_id, LEFT(Phone,3) AS 'Area Code', RIGHT(Phone, Length(Phone)-4) AS 'Phone Number'
FROM Authors;