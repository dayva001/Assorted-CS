...
WHERE A.ArtistID = X.ArtistID AND M.MemberID = X.MemberID AND 
...
FROM Members m join XrefArtistsMembers x JOIN Artists a
...
SELECT *
FROM Titles
WHERE LENGTH(Title) >= ALL (SELECT LENGTH(Title) FROM Titles);

#Q6
SET sql_mode = '';

CREATE OR REPLACE VIEW TrackStats AS
SELECT TitleID, TrackNum, TrackTitle, LengthSeconds, (SELECT MAX(LengthSeconds) AS "Max Length"
FROM Tracks t2 WHERE t2.titleID = t1.titleID),
 LengthSeconds- (SELECT MAX(LengthSeconds) AS "Max Length"
FROM Tracks t2 WHERE t2.titleID = t1.titleID) as 'Difference from Max Length'
FROM Tracks t1
