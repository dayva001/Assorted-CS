...
#Q2
INSERT INTO Tracks (TitleID, TrackNum, TrackTitle, LengthSeconds, MP3, RealAud) Values
(4, 11, 'Spring', 150, 1, 0);
SELECT *
FROM Tracks
WHERE TitleID=4;
...
WHERE City = 'Columbus' AND Region = 'OH' AND HomePhone LIKE '277%';
UPDATE Members2
SET WorkPhone = Concat('899', right(WorkPhone,7))
WHERE City = 'Columbus' AND Region = 'OH' AND WorkPhone LIKE '277%';
...
UPDATE Members2
SET SalesID = 2
WHERE Gender = 'F' AND SalesID IN (SELECT SalesID
								   FROM SalesPeople
...
SELECT 13,'Doug and Terry', City, Region, Country,NULL, CURDATE(), NULL
FROM Members
...
#Q8
Insert into XrefArtistMembers
(MemberID, ArtistID, RespParty)
SELECT MemberID, 13 ,True
FROM Members
WHERE FirstName = 'Doug' AND LastName = 'Finney';

Insert into XrefArtistMembers
(MemberID, ArtistID, RespParty)
SELECT MemberID, 13 ,False
FROM Members
WHERE FirstName = 'Terry' AND LastName = 'Irving';

SELECT *
FROM XrefArtistMembers;
