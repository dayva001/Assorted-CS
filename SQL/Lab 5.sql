...
#Q1
SELECT CONCAT_WS(' ',FirstName, LastName) AS 'Full Name', HomePhone
FROM Members;

#Q2
SELECT CONCAT_WS(' ',FirstName, LastName) AS 'Full Name', Format(DateDiff(Now(), Birthday)/365,0) AS 'Age'
FROM Members;

#Q3
SELECT StudioName, Left(Contact,locate(' ', Contact)) AS 'Studio Contact First Name'
FROM Studios;

#Q4
SELECT StudioName, RIGHT(Contact, Length(Contact)-locate(' ',Contact)) AS 'Studio Contact Last Name'
FROM Studios;
...
#Q6
SELECT G.Genre, IFNULL(Title, 'No Titles') AS Titles
FROM Genre G LEFT JOIN Titles T 
ON G.Genre = T.Genre;


#Q7
SELECT CONCAT(UPPER(LEFT(Genre,1)),LOWER(RIGHT(Genre,Length(Genre)-1))) AS 'Genre'
FROM Genre;

#Q8
SELECT G.Genre,
CASE 
	WHEN Title IS NULL THEN 'No Titles'
	ELSE Title
	END AS Titles
FROM Genre G LEFT JOIN Titles T ON G.Genre = T.Genre;

#Q9
SELECT ArtistName, CASE WHEN LeadSource != 'Ad'
	THEN 'Not Ad'
    ELSE 'Ad'
    END AS LeadSource
FROM Artists;
...
#Q10
SELECT A.ArtistName, CONCAT_WS(' ', M.FirstName, M.LastName) AS 'Responsible Party', Format(DateDiff(EntryDate, Birthday)/365,0) AS 'Age at entry date'
FROM Artists A INNER JOIN XrefArtistsMembers X INNER JOIN Members M
ON A.ArtistID = X.ArtistID AND X.MemberID = M.MemberID
WHERE RespParty = true;