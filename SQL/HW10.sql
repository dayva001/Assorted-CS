...
BEGIN 
    ...
    IF rowcount > 0 THEN
        ...
        WHERE pub_id = new.pub_id;
	ELSE
		INSERT INTO Titles_Total(pub_id,total) VALUES(new.pub_id,1);
    END IF;
    
END; //
DELIMITER ;

SELECT * FROM Publisher_Total;
INSERT INTO Titles VALUES('T14','The Joy of Cooking','Cooking','P04',NULL,NULL,NULL,NULL,0);
INSERT INTO Titles VALUES('T15','Coding for Dummies','Self Help','P02',NULL,NULL,NULL,NULL,0);
SELECT * FROM Publisher_Total;
...
SET
    ...
    Message = CASE WHEN new.royalty_share < 0.5
				THEN CONCAT_WS(' ', au_id, 'your royalty share is low')
                ELSE
                CONCAT_WS(' ', au_id, 'your royalty share is high')
END;
$$
DELIMITER ;

INSERT INTO Title_Authors VALUES(1,'A06',1,0.3);
INSERT INTO Title_Authors VALUES(2,'A13',1,1.0);
SELECT * FROM Author_Message;
