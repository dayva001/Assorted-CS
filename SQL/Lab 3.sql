...
WHERE  TitleID IN (SELECT TitleID
				   FROM Titles
                   WHERE Genre = 'metal') AND RealAud = true;
...
WHERE Birthday = (SELECT MIN(Birthday)
				  FROM Members 
                  WHERE Birthday NOT IN (SELECT MIN(Birthday)
										 FROM Members));

#Q11
SELECT Title, TrackTitle
FROM Titles TI INNER JOIN Tracks TR
ON TI.TitleID = TR.TitleID
WHERE StudioID=1;

#Q12
SELECT Title, StudioName
FROM Titles T INNER JOIN Studios S
ON T.StudioID = S.StudioID;

#Q13
SELECT S.FirstName, S.LastName
FROM SalesPeople S INNER JOIN Members M
ON S.SalesID = M.SalesID
WHERE M.LastName = 'Alvarez';

#Q14
SELECT M.FirstName AS 'Member First Name', M.LastName AS 'Member Last Name', S.FirstName AS 'Sales First Name', S.LastName AS 'Sales Last Name'
FROM SalesPeople S INNER JOIN Members M
ON S.SalesID = M.SalesID
WHERE Region = 'CA';

#Q15
SELECT ArtistName, COUNT(T.ArtistID) AS 'Number of Titles Shipped'
FROM Artists A INNER JOIN Titles T
ON A.ArtistID = T.ArtistID
GROUP BY ArtistName
HAVING COUNT(T.ArtistID) > 1;

#Q16
SELECT Title, COUNT(TI.TitleID) AS 'Number of Tracks'
FROM  Titles TI INNER JOIN Tracks TR
ON TI.TitleID = TR.TitleID
GROUP BY Title
HAVING COUNT(TI.TitleID) < 9;