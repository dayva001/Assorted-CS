...
#Q4
SELECT AVG(LengthSeconds)/60 As 'Avg length (mins)', MIN(LengthSeconds)/60 AS 'Min length (mins)', MAX(LengthSeconds)/60 AS 'Max length (mins)'
FROM Tracks;

#Q5
SELECT SUM(LengthSeconds)/60 AS 'Total time of tracks over 150 secs in minutes'
FROM Tracks
WHERE LengthSeconds > 150;

#Q6
SELECT COUNT(TrackTitle) AS 'Titles starting with s', AVG(LengthSeconds) AS 'Average length (s)'
FROM Tracks
WHERE TrackTitle LIKE 's%';

#Q7
SELECT COUNT(TrackTitle) AS 'Tracks in title 4', SUM(LengthSeconds) AS 'Total title length (s)', AVG(LengthSeconds) AS 'Average track length (s)'
FROM Tracks
WHERE TitleID = '4';

#Q8
SELECT COUNT(*) AS 'Male members in the US'
FROM Members
WHERE Gender = 'M' AND Country = 'USA';

#Q9
SELECT TitleID, COUNT(*) AS 'Number of tracks in title'
FROM Tracks
GROUP BY TitleID;

#Q10
SELECT TitleID, SUM(LengthSeconds)/60 AS 'Total track length (mins)'
FROM Tracks
GROUP BY TitleID;
...
#Q12
SELECT Region, Gender, COUNT(*) AS 'Members'
FROM Members
GROUP BY Region, Gender
ORDER BY Region;

#Q13
SELECT TitleID, AVG(LengthSeconds)/60 AS 'Average track length (mins)', MIN(LengthSeconds)/60 AS 'Shortest track (mins)', MAX(LengthSeconds)/60 AS 'Longest track (mins)'
FROM Tracks
GROUP BY TitleID;

#Q14
SELECT LeadSource, COUNT(LeadSource) AS 'NumArtists', Min(EntryDate) AS 'Earliest entry date', MAX(EntryDate) AS 'Most recent entry date'
FROM Artists
GROUP BY LeadSource;
...
#Q17
SELECT TitleID, COUNT(*) AS NumTracks
FROM Tracks
GROUP BY TitleID
HAVING COUNT(*) < 9;

#Q18
SELECT Region, COUNT(*) AS 'Members with e-mail'
FROM Members
