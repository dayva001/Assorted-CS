...
WHERE LeadSource = 'Email' AND T.ArtistID IS NULL;

#Q4
SELECT ArtistName, StudioID
FROM Artists A LEFT JOIN Titles T
ON A.ArtistID = T.ArtistID;

#Q6
SELECT M.FirstName AS 'Member First Name' , M.LastName AS 'Member Last Name', S.FirstName AS 'Primary First Name',
S.LastName AS 'Primary Last Name', Sup.FirstName AS 'Sec First Name', Sup.LastName AS 'Sec Last Name'
FROM Members M INNER JOIN SalesPeople S INNER JOIN SalesPeople Sup
ON M.SalesID = S.SalesID AND S.Supervisor = Sup.SalesID;

#Q7
SELECT S.FirstName, S.LastName
FROM SalesPeople S RIGHT JOIN SalesPeople Sup
ON S.Supervisor = Sup.SalesID
WHERE Sup.Supervisor IS NULL;
...
#Q9
SELECT Title, StudioName, ArtistName, COUNT(TR.TitleID) AS 'Tracks in Title'
FROM Titles T INNER JOIN Studios S INNER JOIN Artists A INNER JOIN Tracks TR
ON T.StudioID = S.StudioID AND T.ArtistID = A.ArtistID AND T.TitleID = TR.TitleID
...
#Q11
SELECT G.Genre, SUM(LengthSeconds)/60 AS 'Total Length'
FROM Genre G LEFT JOIN (Titles T INNER JOIN Tracks TR)
ON G.Genre = T.Genre AND T.TitleID = TR.TitleID
GROUP BY G.Genre;

#Q12
SELECT FirstName, LastName, ArtistName
FROM Members M INNER JOIN XrefArtistsMembers X INNER JOIN Artists A
ON M.MemberID = X.MemberID AND X.ArtistID = A.ArtistID
WHERE X.RespParty = TRUE;