...
DROP TABLES IF EXISTS Inventory, Warehouses, Products;
...
#Q3
SELECT Gender, COUNT(Gender) AS 'Number of Gender'
FROM Members
GROUP BY Gender
UNION
...
#Q4
SELECT YEAR(EntryDate) AS 'Year', COUNT(YEAR(EntryDate)) AS 'Number of artists entered that year'
FROM Artists
GROUP BY YEAR(EntryDate)
UNION
SELECT 'Total',COUNT(YEAR(EntryDate))
FROM Artists;

#Q5
SELECT TitleID, SUM(MP3)+SUM(RealAud) as 'Num. Audio Files'
FROM Tracks
GROUP BY TitleID
ORDER BY TitleID;
...
(SELECT AVG(LengthSeconds)FROM Tracks WHERE TitleID =1) AS 'Average Length',
 LengthSeconds-(SELECT AVG(LengthSeconds)FROM Tracks WHERE TitleID =1) AS 'Difference from Avg Length'
FROM Tracks
WHERE TitleID =1;

#Q7
SELECT TitleID, TrackNum, TrackTitle, LengthSeconds,
(SELECT AVG(LengthSeconds)FROM Tracks AL WHERE AL.TitleID =T.TitleID) AS 'Average length in title',
LengthSeconds-(SELECT AVG(LengthSeconds)FROM Tracks AL WHERE AL.TitleID =T.TitleID) AS 'Difference from avg length'
FROM Tracks T
...
#Q9
SELECT Title, COUNT(TrackNum) AS 'Number of Tracks', FORMAT(SUM(LengthSeconds)/60,0) as 'Length of Title (mins)'
FROM Titles T LEFT JOIN Tracks TR
...
#Q11
SELECT A.ArtistName, Concat(FirstName, ' ', LastName) AS 'Member Name'
FROM (Artists A LEFT JOIN XrefArtistsMembers X ON A.ArtistID = X.ArtistID) LEFT JOIN Members M
ON X.MemberID = M.MemberID
UNION
SELECT ArtistName, Count(MemberID)
FROM Artist A INNER JOIN XrefArtistsMembers X
ON X.ArtistID=F.ArtistID
GROUP By F.ArtistID
...
SELECT Genre
FROM Genre G
WHERE NOT EXISTS( SELECT GENRE FROM Titles T WHERE T.Genre=G.Genre);

#Q13
SELECT  DISTINCT A.ArtistID, ArtistName
FROM Artists A INNER JOIN (SELECT ArtistID
FROM Members M INNER JOIN
XrefArtistsMembers X
ON M.MemberID = X.MemberID
WHERE M.Country != 'USA') SC
ON A.ArtistID = SC.ArtistID;

-- DROP TABLE IF EXISTS AudioFiles;

-- CREATE TABLE AudioFiles ( TitleID int NOT NULL, TrackNum smallint NOT NULL, AudioFormat boolean NULL);

-- INSERT INTO AudioFiles
-- (TitleID, TrackNum, AudioFormat)
-- SELECT TitleID, TrackNum, MP3
-- FROM Tracks WHERE MP3 = 1;
